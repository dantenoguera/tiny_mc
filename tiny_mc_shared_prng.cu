...
__global__ void setup_prng(curandState * prng_states) {
    if (threadIdx.x == 0) {
        curand_init(1234, blockIdx.x, 0, &prng_states[blockIdx.x]);
    }
}

__global__ void photon(float * heat, float * heat2, curandState * prng_states) {
    __shared__ curandState prng_state;
    if (threadIdx.x == 0) {
        prng_state = prng_states[blockIdx.x];
    }

    __syncthreads();

    curandState local_prng_state = prng_state;
    skipahead(threadIdx.x * 4 * WORK * 1024ull, &local_prng_state);

    __shared__ float _heat[SHELLS];
    __shared__ float _heat2[SHELLS];
    ...
    __syncthreads(); // wait initialization
    ...
    for (int i = thread_from; i < thread_to; ++i) {
        ...
        for (;;) {
            ...
            atomicAdd(&_heat2[shell], (1.0f - albedo) * (1.0f - albedo) * weight * weight);
            weight *= albedo;
...
int main() {
    ...
    checkCudaCall(cudaMalloc((void **)&prng_states,
                block_count * sizeof(curandState))); 
    ...
    printf("Device allocated: %f GB\n", float(sizeof(curandState) * block_count) / (1024 * 1024 * 1024));
    printf("Work: %d\n", WORK);
#else
    printf("# Photons per second (Total) = %lf K\n", 1e-3 * PHOTONS / cpu_elapsed);
#endif
